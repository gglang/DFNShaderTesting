...
Shader "Custom/FX_Shader_FireCracks" {
    Properties {
        ...
        _DisplacementMask ("DisplacementMask", 2D) = "white" {}
    ...
    SubShader {
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                float2 node_8889 = ((float2(_Lava1XSpeed,_Lava1YSpeed)*node_5649.g)+o.uv0);
                float4 _Lava1_var = tex2Dlod(_Lava1,float4(TRANSFORM_TEX(node_8889, _Lava1),0.0,0));
                float2 node_2318 = (o.uv0+(node_5649.g*float2(_Lava2XSpeed,_Lava2YSpeed)));
                float4 _Lava2_var = tex2Dlod(_Lava2,float4(TRANSFORM_TEX(node_2318, _Lava2),0.0,0));
                float4 _DisplacementMask_var = tex2Dlod(_DisplacementMask,float4(TRANSFORM_TEX(o.uv0, _DisplacementMask),0.0,0));
                v.vertex.xyz += ((_Lava1_var.rgb*_Lava2_var.rgb)*_DisplacementMask_var.rgb*float3(0,-7,5));
...
////// Emissive:
                ...
                float3 node_69 = (((((_Lava1Color.rgb*_Lava1_var.rgb)+(_Lava2_var.rgb*_Lava2Color.rgb))*_FirePitAlpha_var.rgb)+(i.vertexColor.rgb*__MainTex_var.rgb))+node_5198);
                float3 emissive = node_69;
                float3 finalColor = emissive;
                float node_5375 = dot(saturate((node_5198+(saturate((i.vertexColor.a*2.0))*__MainTex_var.a))),float3(0.3,0.59,0.11));
                fixed4 finalRGBA = fixed4(finalColor,node_5375);
